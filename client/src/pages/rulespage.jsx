            ...
            {curentUser && curentUser.route == 'admin' ?
                <div className='rule-pages-container'>
                    <div className="container-rule">
                        ...
                        <input onChange={(e) => {(allRules.minAge<=e.target.value)? hanndleChange(e):alert('tuổi cao nhất không phù hợp') }} type="number" name='maxAge' value={allRules ? allRules.maxAge : null} />
                    </div>
                    <div className="container-rule">
                    ...
                    <div className="container-rule">
                        <button className='btn-update-three-rule' onClick={()=>{console.log(allRules);updateallrules();alert('Cập nhật tuổi điểm tối đa tối thiểu sĩ số tối đa')}}>Cập nhật 3 nội dung trên</button>
                    </div>
                    ...
                    <div className="container-rule">
                            ...
                            {allClasses.map((item, index) => {
                                return (
                                    <div className='container-all-class'>
                                        ...
                                        <button className='button-delete-class' onClick={() => { alert('Xóa lớp'); handledeleteclasses(item.LopID);reloadPage() }}>Xóa lớp</button>
                                    </div>
                                )
                            })}
                        </div>
                        <div className="add-class-container">
                            ...
                            <button onClick={() => { alert(`Thêm lớp ${addedClasses}`); handleaddclasses(addedClasses) ;reloadPage()}}>thêm lớp</button>
                            <button className='button-update-subject-and-class' onClick={() => { alert('Xử lý cập nhật lớp '); handleupdateallclass() }}>cập nhật lớp</button>
                        </div>
                    </div>
                    ...
                    <div className="container-rule">
                            ...
                            {allSubject.map((item, index) => {
                                return (
                                    item.TenHK == 'HK1' ? <div className='container-all-class'>
                                        ...
                                        <button className='button-delete-class' onClick={() => { alert('Xóa môn'); console.log(allSubject);handledeletesubject(item.NameID);reloadPage() }}>Xóa môn</button>
                                    </div> : null
                                )
                            })}
                        ...
                        <div className="add-class-container">
                            ...
                            <button className='button-update-subject-and-class' onClick={() => { alert('Đã cập nhật môn '); handleupdateallsubject() }}>Cập nhật môn</button>
                        </div>
                    </div>
